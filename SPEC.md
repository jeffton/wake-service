...
We will add a regular JSON format to the output in addition to the existing compact format for Garmin. Add GET parameter format=json or compact, default json. The purpose of this is to make the output readable by an AI agent in addition to its current use case. Each JSON forecast entry is an object with named keys (time, seaTemperature, waveHeight, waveDirection, temperature, windSpeed, windDirection, cloudCover, condition, uvIndex, precipitation1hours, precipitationAmount1hours, precipitation12hours). Hourly precipitation fields are included only when available upstream. In JSON, cloudCover should be an object with total/low/medium/high fields and time should be formatted in local date/time. Include a meta.units object describing units in JSON output. The compact format should omit cloud cover and hourly precipitation, retain its existing field layout, and stay at 12 entries, while JSON should return all available forecast times. lat/lon must be provided; no default coordinates.
